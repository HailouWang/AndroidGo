...
class ParentRecyclerView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    override fun dispatchTouchEvent(ev: MotionEvent?): Boolean {
        ...
        Log.d("hlwang", "ChildRecyclerView dispatchTouchEvent lastY : " + lastY
                +", e.y : "+ev?.y
                +", isScrollEnd() : "+isScrollEnd()
                +", deltaY : "+(lastY - (ev?.y?:0f).toInt()))
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        Log.d("hlwang", "ChildRecyclerView onTouchEvent lastY : " + lastY
        +", e.y : "+e.y
        +", isScrollEnd() : "+isScrollEnd()
        +", deltaY : "+(lastY - e.y).toInt());
        if(lastY == 0f) {
            lastY = e.y
        }
        if(isScrollEnd()) {
